...
class Portfolio():
    ...
    def track_progress(self, printing, message_optimization, risky):
        ...
        messages.append(" Portfolio Annual Return (252 days)  = {} % ".format(
            round(annual_return * 100, 3)))
        messages.append(" Portfolio Annual Standard Deviation  (252 days)  = {}  ".format(
            round(annual_std, 3)))
        messages.append(" Portfolio Annual Sharpe Ratio  (252 days)  = {}  ".format(
            round(annual_sr, 3)))

        if printing:
            utils.display(weights_df.T)
        if printing:
    ...
    def plot_portfolio_simulation(self, title, date_range, std_arr, ret_arr, sharpe_arr, descriptive_df, returns_merged):
        ...
        plt.scatter(std_arr, ret_arr, c=sharpe_arr,
    ...
    def plot_initial_portfolios(self, title, portfolio_arr, descriptive_df):
        ...
        plt.scatter(std_arr, ret_arr, c='y', marker='o',
                    edgecolors="grey", s=100, alpha=0.5, linewidth=2)
        for i, txt in enumerate(portfolio_arr):
            plt.annotate(
                txt, (std_arr[i] + 0.01, ret_arr[i] - 0.003), size=7.5)
            plt.annotate("SR = {}".format(
